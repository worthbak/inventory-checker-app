...
extension SKUData {
    init(from skus: [ProductSku]) {
        orderedSKUs = skus.map { $0.partNumber }
        skuLookup = skus.reduce(into: [:], { partialResult, next in
            partialResult[next.partNumber] = next.displayName
        })
    }
}
